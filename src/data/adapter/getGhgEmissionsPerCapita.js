import data from "../dataset/ghg/emissions/ghg-emissions-per-capita.json";
import countries from "i18n-iso-countries";
countries.registerLocale(require("i18n-iso-countries/langs/en.json"));
countries.registerLocale(require("i18n-iso-countries/langs/fr.json"));

const getGhgEmissionsPerCapita = (selectedCountries, year) => {
        ...
        data.filter(datum => parseInt(datum.Year) === parseInt(year)).forEach(datum => {
            if (selectedCountries.includes(countries.getName(datum.Code, "fr", {select: "official"}))) {
                chartData
                    ...
                    .y = parseFloat((datum['Total including LUCF (per capita) (CAIT, 2020)']).toFixed(2))
            }
        })
    ...
    data.forEach(datum => {
        if (selectedCountries.includes(countries.getName(datum.Code, "fr", {select: "official"}))) {
            chartData
                .find(chartDatum => chartDatum.name === countries.getName(datum.Code, "fr", {select: "official"}))
                .data.push(parseFloat((datum['Total including LUCF (per capita) (CAIT, 2020)']).toFixed(2)))
        }
    })
...
export default getGhgEmissionsPerCapita